...
個人投資組合風控與即時觀點儀表板（互動版）。整合台股 + 美股部位，提供 Investment Memo、族群佔比、集中度警示、壓力測試、催化劑日曆等功能。

## 🔒 隱私說明

這份 repo **不包含任何使用者真實部位資料**。預設股數、成本、現金皆為 0。所有真實資料儲存在使用者自己瀏覽器的 localStorage（不會上傳到任何伺服器、不會被 commit 進 git）。

如需在多裝置間同步資料，使用工具列的「📥 匯出 JSON」與「📤 匯入 JSON」即可。
